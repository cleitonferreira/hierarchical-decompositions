...
public class AlgoritmoCNMLL extends AlgoritmoAbstract {
  ...
  public SolucaoCNMLL buscaJuncaoGrupos(ArrayList<Integer> clustersAtivos, SolucaoCNMLL solucao) {
    ...
    RetornoSolucao retornoSolucao = ((CalculadorCNM) this.calculador).calculateFormulaComplexidadeEgravaEstado(
        solucao, solucao.getValores());
    double fitnessMelhor = retornoSolucao.getValorFormulaComplexidade();
    ...
    for (int i = 0; i < clustersAtivos.size() - 1; i++) {
      for (int j = i + 1; j < clustersAtivos.size(); j++) {
        ...
        double fitnessJuncao = ((CalculadorCNM) this.calculador).evaluateJoinOtimizado(solucao,
            cluster1, cluster2);
