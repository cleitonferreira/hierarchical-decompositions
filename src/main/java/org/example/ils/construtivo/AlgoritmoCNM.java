...
public class AlgoritmoCNM extends AlgoritmoAbstract {
  ...
  protected void iniciaSolucao(SolucaoCNM solucao) {
    ...
    RetornoSolucao retornoSolucao = this.calculador.evaluate(solucao, valores);
    double fitness = retornoSolucao.getValorFormulaComplexidade();
    int length = Arrays.stream(valores).distinct().toArray().length;
    int[] grupos = new int[length];
    solucao.setSolucao(valores, qtdItens, totalGrupos, fitness, 0, grupos);
  }
  ...
  public SolucaoCNM buscaJuncaoGrupos(SolucaoCNM solucao) {
    ...
    for (int i = 0; i < totalGrupos - 1; i++) {
      for (int j = i + 1; j < totalGrupos; j++) {
        ...
        this.evaluation++;
        RetornoSolucao retornoSolucaoJuncao = this.calculador.evaluate(solucao, valores);
        double fitnessJuncao = retornoSolucaoJuncao.getValorFormulaComplexidade();
        ...
        if (fitnessJuncao < fitnessMelhor) {
          ...
          grupos = retornoSolucaoJuncao.getGrupos();
        }
      }
    }
		if (!encontrouMelhor) {
    ...
    SolucaoCNM solucaoMelhor = new SolucaoCNM(valores, qtdItens, totalGrupos - 1, fitnessMelhor,
        location, grupos);
