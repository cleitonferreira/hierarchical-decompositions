...
public class AlgoritmoHC extends AlgoritmoAbstract {
    ...
    for (int i = 0; i < totalItens; i++) {
      ...
      for (int j = 0; j < totalGrupos; j++) {
        if (iGrupoAtual != j) {
          ...
          RetornoSolucao retornoSolucao = ((CalculadorHC) calculador).evaluateEGravaEstado(
              solucaoMelhor, valores);
          ...
          if (fitness < melhorFitness) {
            ...
            grupos = retornoSolucao.getGrupos();
          }
        }
      }
  ...
  public void executa(String... args) {
    ...
    RetornoSolucao retornoSolucao = ((CalculadorHC) this.calculador).evaluateEGravaEstado(solucao,
        solucao.getValores());
    double fitness = retornoSolucao.getValorFormulaComplexidade();
