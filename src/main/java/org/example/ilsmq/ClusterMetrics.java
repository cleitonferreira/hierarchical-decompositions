...
public class ClusterMetrics {
  ...
  private ClusterMetrics(HMD hmd) {
    this.hmd = hmd;
  }

  public ClusterMetrics(HMD hmd, int[] solution) {
    this.hmd = hmd;
  ...
  public void makeMoviment(int module, int toCluster) {
    ...
    if (fromCluster != -1) {
      ...
      modularizationFactor[fromCluster] =
          calculateClusterModularizationFactor(
              internalDependencyWeight[fromCluster], externalDependencyWeight[fromCluster]);
    ...
    if (toCluster != -1) {
      ...
      modularizationFactor[toCluster] =
          calculateClusterModularizationFactor(
              internalDependencyWeight[toCluster], externalDependencyWeight[toCluster]);
  ...
  public double calculateMovimentDelta(int module, int toCluster) {
    ...
    double MFAfter =
        calculateClusterModularizationFactor(metrics[0], metrics[1])
            + calculateClusterModularizationFactor(metrics[2], metrics[3]);
      ...
      for (int i = 0; i < moduleDependenciesCount(module); i++) {
        ...
        if (depModule == module) { // modulo dependente de si
          ...
          toInternalDependencyWeight += depWeight;
            ...
            == fromCluster) { // modulos estavam no mesmo cluster -> serão separados
          ...
          toExternalDependencyWeight += depWeight; // passará a ser externa nos dois clusteres
            ...
            == toCluster) { // modulos estavam separados -> ficarão no mesmo cluster
          fromExternalDependencyWeight -=
              depWeight; // dependencia deixará de ser externa nos dois clusteres
          toExternalDependencyWeight -=
              depWeight; // dependencia deixará de ser externa nos dois clusteres
          toInternalDependencyWeight += depWeight; // passará a ser interna no cluster de destino
        } else
        /* if(depCluster != fromCluster && depCluster != toCluster) */ { // dependência com módulos
                                                                         // que não
          // pertencem nem a origem nem ao destino
          // tem que ser transferidas de um
          // módulo ao outro
          fromExternalDependencyWeight -=
              depWeight; // cluster de origem deixa de ter a dependência externa
          toExternalDependencyWeight +=
              depWeight; // cluster de destino passa a ter a dependência externa
        }
      }
    }

    return new int[] {
      fromInternalDependencyWeight,
      fromExternalDependencyWeight,
      toInternalDependencyWeight,
      toExternalDependencyWeight
    };
  }
  ...
  public double calculateMergeClustersDelta(int cluster1, int cluster2) {
    ...
    int joinClusterInternalDependency =
        internalDependencyWeight[cluster1] + internalDependencyWeight[cluster2];
    int joinClusterExternalDependency =
        externalDependencyWeight[cluster1] + externalDependencyWeight[cluster2];
    ...
    for (int i : modulesOnCluster.get(cluster1)) {
      for (int j : modulesOnCluster.get(cluster2)) {
        ...
        joinClusterExternalDependency -=
            dependencyEachOtherWeight; // aresta externa deixou de existir (no
        // cluster1)
        joinClusterExternalDependency -=
            dependencyEachOtherWeight; // aresta externa deixou de existir (no
        // cluster2)
  ...
  public int smallestClusterMFNumber() {
    ...
    if (totalClusteres == 0) return convertToClusterNumber(0);
    return cluster;
  }
  ...
  public ClusterMetrics clone() {
    ClusterMetrics cm = new ClusterMetrics(hmd);
    ...
    System.arraycopy(
        this.totalModulesOnCluster,
        0,
        cm.totalModulesOnCluster,
        0,
        this.totalModulesOnCluster.length);
